...
# ok='F:/hepa2/ok total';setwd(dir=ok)#'D:/Data for Seurat Analysis/filtered_gene_bc_matrices/Data Filtered and Processed in Seurat/Corrected/E11/';setwd(dir=ok)#o
# E11_big=readRDS('E11.5_big_ok Tikka 15.5.23 .rds') # E11_big3=readRDS('E11.5_grant int_ptt30323.rds') #in 'E11.5_big Tikka 15.5.23 .rds' / 'E11.5_big_ok Tikka 15.5.23 .rds'
# DimPlot( E11_big, reduction = "umap",pt.size = 1.5, label = TRUE, label.size =14 ,group.by = 'seurat_clusters') + ggtitle('') + guides(colour = guide_legend(override.aes = list(size=8)))
# vars=c('Pitx2','Sox2','Shh','Acta2');FeaturePlot(E11_big, features = vars, pt.size = 4,reduction='umap',order=TRUE, cols = c("#c0c0c0",'red'))
# E11_epit=readRDS('E11.5_epithelia Tikka 20.7.23 .rds')
# 
# # ok='D:/Data for Seurat Analysis/filtered_gene_bc_matrices/Data Filtered and Processed in Seurat/Corrected/E14/news/';setwd(dir=ok)#o
# # ok='F:/ok total';setwd(dir=ok)
# E14_big=readRDS('E14.25_6v_3500g_11p_.2r_20n_.1d_cut Tikka 18.7.23 .rds')
# DimPlot( E14_big, reduction = "umap",pt.size = 1.5, label = TRUE, label.size =14 ,group.by = 'seurat_clusters') + ggtitle('') + guides(colour = guide_legend(override.aes = list(size=8)))
# E14_epit=readRDS('E14.25_epithelia_v2 Tikka 24.7.23 .rds')
# 
# #Same for E16:
# # ok='D:/Data for Seurat Analysis/filtered_gene_bc_matrices/Data Filtered and Processed in Seurat/Corrected/E14/Integration/';setwd(dir=ok)#o
# # E16_big=readRDS('E16.5_double int_r0.9_pc18_ptt31323.rds')
# # ok='D:/Data for Seurat Analysis/filtered_gene_bc_matrices/Data Filtered and Processed in Seurat/Corrected/E16/new2';setwd(dir=ok)#or lapimeno E11-16_Pitx2vmeo2 or E11_amap Mesenchyme#The below works if ensemble looks like:
# E16_big=readRDS('E16.5_big_ok Tikka 19.7.23 .rds')
# DimPlot( E16_big, reduction = "umap",pt.size = 1.5, label = TRUE, label.size =14 ,group.by = 'seurat_clusters') + ggtitle('') + guides(colour = guide_legend(override.aes = list(size=8)))
# E16_epit=readRDS('E16.5_epithelia Tikka 24.7.23 .rds')
# 
# #Mesenchymes #All:
# # ok='D:/Data for Seurat Analysis/filtered_gene_bc_matrices/Data Filtered and Processed in Seurat/Mesenchyme'; setwd(dir=ok)
# # mes1=readRDS("data_e11_mese_tikka24323.rds"); # mes2=readRDS("data_e14_mese_tikka24323.rds"); # mes3=readRDS("data_e16_mese_tikka24323.rds");#Msx1 reduced:
# # mes1a=readRDS("data_e11_M_Msx1_tikka23323.rds"); # mes2a=readRDS("data_e14_M_Msx1_tikka23323.rds"); # mes3a=readRDS("data_e16_M_Msx1_tikka23323.rds"); 
# 
# #tot
# vari=readRDS("E11-16_c15_Mes_ptt24323.rds")
# DimPlot(mes1a, reduction = "umap",pt.size = 1.5, label = TRUE, label.size =14 ,group.by = 'seurat_clusters') + ggtitle('') + guides(colour = guide_legend(override.aes = list(size=8)))
# #Less clusters:
# data1=vari; vari <- FindClusters(vari, resolution = 0.15) 
...
# DefaultAssay( vari) <- "RNA"; mes1aa <- subset(x = vari, idents = c(1,2,3,4))
# e1=subset(x = mes1aa, subset = UMAP_2 < 5);c0=names(mes1aa@active.ident) %in% colnames(e1); mes1aa=mes1aa[,c0];
# DimPlot(mes1aa, reduction = "umap",pt.size = 3.5, label = TRUE, order=FALSE)
# name='data_e11_mese_4clust_tikka29323.rds'; saveRDS(mes1aa, file = name)

#Rishi:setwd(dir='F:/Processed Data'); scObj = readRDS(file="scaled_Aggregated_seurat.rds"); data2e=scObj
```
...
#For this you need to have Datalist, data, the name for saving, and mc for number of cell in minimum sample (if less than 100, and if above then 100)
DefaultAssay(data1a) <- "RNA"; DataList1 <- SplitObject(data1a, split.by = "orig.ident") #DataList is made from the filtered data, but check manually if any of the sets are less than 80 
DefaultAssay(data2a) <- "RNA"; DataList2 <- SplitObject(data2a, split.by = "orig.ident") #DataList is made from the filtered data, but check manually if any of the sets are less than 80 
DefaultAssay(data3)  <- "RNA"; DataList3 <- SplitObject(data3, split.by = "orig.ident")
...
#this is now second version of the total merge... with transfrom2 and filter to two experiments otherwise differently
date1[[1]]; date1[[2]]; date1[[3]]; Data1=date1[[1]]; Data1[c('Sox2','Pitx2'),]
