...
xmi;xma;ymi; yma #check that these are ok
a=xlim(xmi,xma); b=ylim(ymi, yma)
for (i in 1:(prints-1)) {jpeg(paste(nmj,"UMAPs of interesting genes",i,".jpg"), width = width, height = height, 
                              quality = 100,pointsize = 1, res=1200); #width = 12000, height = 9700, CHECK X AND YLIMS!!!
  prt=FeaturePlot(lena, reduction = "umap",features = vt[(16*i-16+1):(16*i)],slot='scale.data',
  ...
  print(prt);dev.off()} # https://github.com/satijalab/seurat/issues/1080 to print just one scale automatically...
...
setwd(dir=ok); write.csv(marker_cosg, paste('Epit combo2_v2 DE genes_COSG_',one,def,namedate,'.csv'),quote = F); 
...
var=unique(vtg2); namee='with de genes by COSG';setwd(dir=ok); #genes of interest , de genes
...
var=unique(siis); namee='with de genes by COSG';setwd(dir=ok); #genes of interest , de genes
...
var=unique(vars_tt);setwd(dir=ok); #genes of interest , de genes
pb11a <- ScaleData(vari,features = rownames(vari),assay='RNA') ; #If you do not get any individual gene hits with SCT assay, RNA assay and scaling might help with heatmap plotting..
pb11a@meta.data$seurat_clusters <- droplevels(pb11a@meta.data$seurat_clusters)#https://github.com/satijalab/seurat/issues/5069, you may need d
...
h=41000; w=12000 
var=unique(list);setwd(dir=ok); #genes of interest , de genes
